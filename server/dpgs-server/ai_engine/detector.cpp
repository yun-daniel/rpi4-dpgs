...
//    const int dim = OUTPUT_DIM;
//    const int rows = TOTAL_PREDICTIONS;
    const int dim = outputs[0].size[2];
...
void detector(cv::Mat &_frame, cv::dnn::Net &net, std::vector<Detection> &output, const std::vector<std::string> &class_name) {
    ...
    blob = cv::dnn::blobFromImage(frame, 1.0/255.0, cv::Size(INPUT_WIDTH, INPUT_HEIGHT), cv::Scalar(), true, false);
