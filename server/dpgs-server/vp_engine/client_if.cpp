...
void ClientIF::recv_image(){
    while(true){
        unique_lock<mutex> lock(VPEngine::queue_mutex);
        VPEngine::queue_cv.wait(lock, []{ return !VPEngine::frame_queue.empty(); });

        Mat processed = VPEngine::frame_queue.front();
        VPEngine::frame_queue.pop();
        lock.unlock();
