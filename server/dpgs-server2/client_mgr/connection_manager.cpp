...
bool ConnectionManager::initialize(ClientManager* _clt_mgr, StrFrameBuffer* _fb) {
    ...
    if (_fb == nullptr) {
        fprintf(stderr, "Failed to connect fb\n");
        return false;
    }
    else {
        fb_ptr = _fb;
...
void ConnectionManager::exec_client_thread(int clnt_sock) {
    ...
    rd_ptr->sm_ptr = new StreamingModule(*fb_ptr);
